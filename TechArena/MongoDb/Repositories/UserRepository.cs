...
namespace TechArena.MongoDb.Repositories
{
    public class UserRepository
    {
        ...
        public async Task<IUser?> GetByIdAsync(string id) => await _collection.Find(p => p.Id == id).FirstOrDefaultAsync();

        public async Task CreateAsync(IUser pantry) => await _collection.InsertOneAsync(pantry);

        public async Task UpdateAsync(string id, IUser pantry) => await _collection.ReplaceOneAsync(p => p.Id == id, pantry);
